...
public:
  ...
  void unprojDepth(double *depth, int gWidth, int gHeight, double *xdata, double *ydata, double *zdata);
	void setViewpoint(double _azimuth, double _elevation, double _yaw, double _distance, double _fieldOfView);

  const osg::Matrixd& getViewMatrix();
  const osg::Matrixd getProjectionMatrix();
  const osg::Matrix computeMVPW();
...
MEX_DEFINE(unprojDepth) (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) 
{
	InputArguments input(nrhs, prhs, 2);
  ...
  // parse input
	CLR::Renderer* renderer = Session<CLR::Renderer>::get(input.get(0));
  double *pdepth = mxGetPr(prhs[1]);
  int width = mxGetN(prhs[1]);
  int height = mxGetM(prhs[1]);

  // parse output
  plhs[0] = mxCreateDoubleMatrix(height, width, mxREAL);
  double *xdata = mxGetPr(plhs[0]);
  plhs[1] = mxCreateDoubleMatrix(height, width, mxREAL);
  double *ydata = mxGetPr(plhs[1]);
  plhs[2] = mxCreateDoubleMatrix(height, width, mxREAL);
  double *zdata = mxGetPr(plhs[2]);

  // unproject
  renderer->unprojDepth(pdepth, width, height, xdata, ydata, zdata);
	output.set(0,plhs[0]);
	output.set(1,plhs[1]);
	output.set(2,plhs[2]);
}
