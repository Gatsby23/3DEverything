...
{
  osg::Matrix matrix(viewer->getCamera()->getProjectionMatrix());

  if (viewer->getCamera()->getViewport())
      matrix.postMult(viewer->getCamera()->getViewport()->computeWindowMatrix());
  else
      OSG_WARN<<"osg::Matrix Renderer::computeMVPW() - error no viewport attached to SceneView, coords will be computed inccorectly."<<std::endl;
...
void Renderer::flipDepth(float * depthInput, int gWidth, int gHeight, double * depthOutput)
{
	for (int x = 0; x < gWidth; x++)
  {
		for (int y = 0; y < gHeight; y++) 
    {
      ...
      depthOutput[x*gHeight+y] = (double)depthInput[y*gWidth+x];
		}
	}
}


const osg::Matrix Renderer::computeMVPW()
{
    osg::Matrix matrix( viewer->getCamera()->getViewMatrix() * viewer->getCamera()->getProjectionMatrix());
...
// backproject depth into 3D points
void Renderer::unprojDepth(double *depth, int width, int height, double *xdata, double *ydata, double *zdata)
{
	...
	for (int x = 0; x < width; x++) 
  {
    ...
    {
			if(depth[x*height+y] < 1 && depth[x*height+y] > 0)
      {
      	osg::Vec3 window = osg::Vec3(x, y, depth[x*height+y])*inverseMVPW;
			  xdata[x*height+y] = (double)window.x();
			  ydata[x*height+y] = (double)window.y();
        zdata[x*height+y] = (double)window.z();
      }
		}
	}
}
