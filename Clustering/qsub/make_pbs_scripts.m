function make_pbs_scripts

% load filenames
filename = '../sift_features/filenames.mat';
...
for o_i = 1:num_job
  ...
  fprintf(fid, 'cd /scail/scratch/u/yuxiang/3DEverything/Clustering\n');
  if is_multiple
      if o_i == num_job
  ...
  fprintf(fid, ['matlab.new -nodesktop -nosplash -r "compute_sift_matching(' s '); exit;"']);
  fclose(fid);
end
